...
use "$projdir/dta/src/ENNVIH/ehh02dta_all/ehh02dta_b2/ii_su1.dta", clear

egen ag_sale_inc = rowtotal(su161 su162 su163)
...
* Rural income (INR):
use "$projdir/dta/src/ENNVIH/ehh02dta_all/ehh02dta_b2/ii_inr.dta", clear
...
* Non-ag business (NNA):
use "$projdir/dta/src/ENNVIH/ehh02dta_all/ehh02dta_b2/ii_nna1.dta", clear
...
replace nonagbiz_earn = . if nonagbiz_earn > 1000000 // 0 obs
replace nonagbiz_earn = . if nonagbiz_earn < -100000 // 2 obs
...
// There are a few very high values seem like errors (all are 1000000000)

foreach var of varlist in* {
	replace `var' = . if `var' == 1000000000
	replace `var' = . if `var' > 1000000 // 2 obs
}
...
) // PROGRESA data excluded from the public datasets in future wavew
...
use "$projdir/dta/src/ENNVIH/ehh02dta_all/ehh02dta_b3b/iiib_tp.dta", clear
...
use "$projdir/dta/src/ENNVIH/ehh02dta_all/ehh02dta_b3b/iiib_th.dta", clear

egen trsf_s = rowtotal(th261 th262 th263 th264 th267_21)
...
use "$projdir/dta/src/ENNVIH/ehh02dta_all/ehh02dta_b3b/iiib_thi.dta", clear

egen trsf_c = rowtotal(thi301 thi302 thi303 thi304 thi307_21)
...
use "$projdir/dta/src/ENNVIH/ehh02dta_all/ehh02dta_b3b/iiib_to.dta", clear
...
save "ennvih_income_hhlvl_02.dta", replace
